...
export default async function StudentCalendarPage() {
  ...
  const assignments = await prisma.assignment.findMany({
    ...
    include: {
      submissions: { where: { studentId: user.id }, include: { grade: true } },
    },
    orderBy: { dueDate: "asc" },
  })
          ...
          <div className="space-y-3">
            {assignments.map(a => (
              <Link key={a.id} href={`/student/assignments/${a.id}`} className="flex items-center gap-4 py-2 hover:bg-gray-50 rounded-lg px-2">
                ...
                <div>
                  {a.submissions[0] ? (
                    <span className="text-xs bg-green-100 text-green-700 px-2 py-1 rounded-full">Здано</span>
                  ) : (
                    <span className="text-xs bg-orange-100 text-orange-700 px-2 py-1 rounded-full">Не здано</span>
                  )}
                </div>
              </Link>
            ))}
