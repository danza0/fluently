  ...
  return (
    <div className="min-h-screen bg-[#FFFDF8]">
      ...
      <div className="w-full bg-[var(--accent-300)] px-4 md:px-8 py-6">
        <Link href="/student" className="inline-flex items-center gap-2 text-[var(--accent-900)]/70 hover:text-[var(--accent-900)] mb-4 text-sm">
      ...
      <div className="max-w-3xl mx-auto px-4 md:px-8 py-6 space-y-4">
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-5 flex items-start gap-4">
          ...
          <div className="flex flex-col items-center justify-center bg-[var(--accent-300)] rounded-xl px-4 py-3 flex-shrink-0">
            ...
            <span className="text-2xl font-bold text-[var(--accent-900)] leading-none">
              {dueDate ? format(dueDate, "d") : "—"}
            </span>
          </div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-5">
          <h2 className="text-base font-semibold text-[#111111] mb-4 flex items-center gap-2">
            <Upload className="w-5 h-5 text-[var(--accent-600)]" />
          ...
          {submission?.attachments?.length > 0 && (
            <div className="mb-4 p-3 bg-[var(--accent-100)] rounded-lg">
              <p className="text-sm font-medium text-[var(--accent-600)] mb-2">Завантажені файли:</p>
              <div className="space-y-2">
                    ...
                    {att.fileType.startsWith("image/") ? (
                      <div>
                        ...
                        <a href={att.fileUrl} target="_blank" rel="noopener noreferrer"
                          className="text-xs text-[var(--accent-600)] hover:underline mt-1 flex items-center gap-1">
          ...
          {!isGraded && (
            <>
              ...
              <div className="mb-4">
                ...
                {files.length > 0 && (
                  <div className="space-y-2 mt-3">
                    {files.map((file, i) => (
                      <div key={i} className="flex items-center gap-2 bg-[var(--accent-100)] px-3 py-1.5 rounded-lg text-sm">
                        {file.type.startsWith("image/") ? <ImageIcon className="w-4 h-4 text-[var(--accent-600)]" /> : <FileText className="w-4 h-4 text-[var(--accent-600)]" />}
              ...
              <div className="flex gap-3">
                <Button onClick={submit} disabled={submitting} className="bg-[var(--accent-300)] hover:bg-[var(--accent-500)] text-[var(--accent-900)] hover:text-white">
